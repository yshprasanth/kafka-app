        ...
        while(true) {
            ...
            future.addCallback(new ListenableFutureCallback<>() {
                public void onSuccess(SendResult<String, Book> result) {
                    System.out.println("onSuccess, message=[" + result.getProducerRecord().value() + "] with offset=[" +
                            result.getRecordMetadata().offset() + "]");
                }

                public void onFailure(Throwable ex) {
                    System.out.println("onFailure, unable to send message=[" + book + "] due to : " + ex.getMessage());
                }
            });
